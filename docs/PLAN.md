...
6. Each fragment is played one, two, or three times. After a cue, voice activity starts and silence stops the answer recording.
...
Auto and Manual are selected on the home screen. Auto mode only needs the initial “ready” button; voice detection runs the challenge after that and plays the reconstructed result after a short reveal. Manual mode keeps automatic playback repeats but requires explicit start, stop, and next-fragment actions for rooms where reliable detection is impossible. A visible timer and adaptive safety limit prevent a forgotten manual take from recording forever. Space triggers the primary action, R handles replay and retakes, P or Play/Pause safely pauses the challenge, and Backspace or Previous Track returns to the previous safe step. A small geometry-based focus controller supports TV remotes with arrows and OK without adding the full catalog-oriented navigation system used by Kino.
...
- Setup can derive that minimum from a three-second sample of the singer's voice.
...
- [x] Add install icons and a 1200 × 630 social sharing image
- [x] Deploy `dist/` to Cloudflare and configure `apoj.saa.sh`
